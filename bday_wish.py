...
def set_custom_theme(collage_file, music_file):
    img_b64 = get_base64_of_file(collage_file)
    music_b64 = get_base64_of_file(music_file)
    # Get the current volume from session state, which is updated by the slider
    vol = st.session_state.get('bg_volume', 0.4)
    bg_image_part = f"url('data:image/jpeg;base64,{img_b64}')" if img_b64 else "none"
    
    # Use f-string formatting for cleaner Python/HTML integration
    html = f"""
    <style>
    ...
    .stApp, p, label, .stMarkdown, .stAlert, .stToast {{font-family: 'Fuzzy Bubbles', cursive; color: #b00072;}}
    .stApp {{background-image: {bg_image_part}; background-size: cover; background-position: center; background-attachment: fixed; background-repeat: no-repeat; background-blend-mode: soft-light; background-color: rgba(255, 245, 247, 0.9);}}
    .main .block-container {{max-width: 800px; width: 95% !important; padding: 20px !important; margin-left: auto; margin-right: auto; background-color: #FFFFFF; border-radius: 25px; box-shadow: 0 8px 20px rgba(255, 105, 180, 0.5); border: 5px solid #ffb6c1;}}
    h1 {{font-family: 'Chewy', cursive; color: #b00072; font-size: 2.5em; text-shadow: 2px 2px 0 #ffe0f0; text-align: center; border-bottom: 4px dashed #ffb6c1; padding-bottom: 10px;}}
    .stButton > button {{background-color: #ffb6c1; color: white !important; border: 2px solid #ff99aa; border-radius: 10px; padding: 8px 15px; font-family: 'Fuzzy Bubbles', cursive; font-size: 0.9em;}}
    .heart-grid-wrapper .stButton > button {{background-color: #ff99aa; border: 1px solid #ffb6c1; border-radius: 8px; padding: 5px 5px; font-size: 0.85em; height: 60px; line-height: 1.2;}}
    </style>
    ...
    (function(){{
        ...
        if (!music) return;
        // Set volume using the latest value from Streamlit state on every app rerun
        try {{ music.volume = {vol}; }} catch(e){{}}

        // Attempt play (may fail due to browser policy)
        music.play().catch(function(){{
            try {{ music.muted = true; music.play().catch(function(){{}}); }} catch(e){{}}
        }});
        ...
        function attemptUnmuteAndPlay(){{ 
            if (music) {{ 
                music.muted = false; 
                try {{ music.play().catch(function(){{}}); }} catch(e){{}} 
            }} 
        }}
...
def page_home():
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    cols3 = st.columns(1)
    cols3[0].button("🤫 Admin View! Not for you.", use_container_width=True, on_click=lambda: set_page('admin_view', 'home'))
...
def page_voice():
    ...
    st.info("💡 **Background Music too loud?** Use the slider below to lower the background volume, then press play on the voice note again!")
    
    current_volume = st.session_state.get('bg_volume', 0.4)
    # FIX: Use 'bg_volume' as the key directly. This saves the value to st.session_state['bg_volume'] and triggers a rerun.
    new_volume = st.slider(
        ...
        step=0.05, 
        format="%.2f", 
        key='bg_volume', # Correct key for direct state update
        help="Adjust this volume. The music will pause when the voice note plays, and resume at this level when it finishes."
    )
    
    # Removed: The complex and unnecessary js_set_volume block. set_custom_theme handles volume update on rerun.

    st.markdown("---")
    
    if voice_path:
        # Note: Streamlit will re-create this component on rerun
        st.audio(voice_path, format="audio/mp3")
        
        # JAVASCRIPT TO PAUSE/RESUME MUSIC
        js_control = """
        <script>
        (function(){
            var attempts = 0;
            var iv = setInterval(function(){
                attempts += 1;
                // Note: window._bday_music is set in set_custom_theme
                var voice = document.querySelector('[data-testid="stAudio"] audio');
                var bg = window._bday_music || document.getElementById('bday-music');
                if (!voice || !bg) { if (attempts>50) clearInterval(iv); return; }
                
                // 1. Pause BG music when voice note plays
                voice.onplay = function(){ try{ bg.pause(); }catch(e){} };
                
                // 2. Resume BG music when voice note ends/pauses
                var resume = function(){ 
                    try{ 
                        if (voice.paused || voice.ended) { 
                            // Use the latest volume setting from the slider (now stored in new_volume)
                            bg.volume = %s; 
                            bg.play().catch(function(){}); 
                        } 
                    }catch(e){} 
                };
                voice.onended = resume;
                voice.onpause = resume;
        ...
        st.error("❌ Error: The voice note file '%s' was not found." % VOICE_FILE)
        st.info("Please make sure 'voice_note.mp3' is in the same folder as your Python script.")
    st.markdown("---")
...
def page_future():
    ...
    try:
        fav = st.session_state.get('favorite_future_image')
        default_index = image_keys.index(fav) if fav in image_keys else 0
    except Exception:
        default_index = 0
    selected_image_caption = st.radio("**Which of these moments is your favorite vision for us?**", options=image_keys, index=default_index, key='future_image_selector')
    ...
    if col_button.button("Save this Future Pic! 💾", use_container_width=True):
        ...
        try: st.toast("Future Pic Saved! ❤️", icon="💾")
        except Exception: pass
        st.rerun()
    if st.session_state.get('favorite_future_image_saved'):
        col_message.success("**Future Pic Saved!** 💖 You chose: **%s**! This will be our official future goal! 🥰" % st.session_state.get('favorite_future_image'))
    st.markdown("---")
...
def page_love_jar():
    ...
    <div style="background-color: #ffefff; padding: 40px; border-radius: 20px; border: 5px solid #ffccff; text-align: center; box-shadow: 0 4px 15px rgba(255, 105, 180, 0.7); color:#b00072;">
        <p style="font-size: 1.5em; color: #b00072; font-weight: bold; margin-bottom: 0; font-family: 'Dancing Script', cursive;">"%s"</p>
        </div>
    """ % st.session_state['current_love_message'], unsafe_allow_html=True)
    st.markdown("---")
    ...
    if st.session_state.get('message_status') == 'sent':
        ...
        with st.form("feedback_form"):
            ...
            if submitted and st.session_state.get('message_status') == 'sent':
                st.rerun()
            elif submitted and not st.session_state.get('message'):
            ...
            for i, q_data in enumerate(THIS_OR_THAT_QUESTIONS):
                ...
                with cols[col_index]:
                    ...
                    try:
                        ...
                        default_index = 0
                    st.radio("Choose One:", options=q_data['options'], index=default_index, key=f"response_{q_data['key']}", label_visibility='collapsed')
                    st.markdown("---")
...
def page_catch_hearts():
    ...
    if 'correct_spot_index' not in st.session_state:
        st.session_state['correct_spot_index'] = random.randint(0, 8)
    def handle_click(clicked_spot_index):
        try:
            idx = int(clicked_spot_index)
        except Exception:
            return
        if idx == st.session_state.get('correct_spot_index'):
            st.session_state['hearts_caught'] = st.session_state.get('hearts_caught', 0) + 1
            st.session_state['correct_spot_index'] = random.randint(0, 8)
            try: st.toast("A heart found! ❤️", icon='🎉')
            except Exception: pass
        else:
            try: st.toast("❌ Oops! Empty. Try again! 💔", icon='❌')
            except Exception: pass
    button_index = 0
    st.markdown('<div class="heart-grid-wrapper">', unsafe_allow_html=True)
    for row in range(3):
        cols = st.columns(3)
        for col_index in range(3):
            with cols[col_index]:
                st.button("Search Here", key=f"grid_spot_{row}_{col_index}", on_click=handle_click, args=(button_index,), use_container_width=True)
            button_index += 1
...
def page_spin_wheel():
    ...
    if 'wheel_state' not in st.session_state:
        st.session_state['wheel_state'] = 'initial'
    ...
    def finalize_spin():
        ...
        try: st.toast("Spin Complete! 🎉", icon='🥳')
        except Exception: pass
    lottie_wheel_html = """
    <div style="display: flex; justify-content: center; margin-bottom: 20px;">
        <script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script>
        <lottie-player id="lottie-wheel" src="https://lottie.host/46497f62-381c-4b68-80af-c119932135c3/tP6n4NlS4n.json" background="transparent" speed="1" style="width: 250px; height: 250px;" loop autoplay></lottie-player>
    </div>
    """
    ...
    if st.session_state['wheel_state'] == 'result' and st.session_state['wheel_result']:
        ...
        st.markdown(f"""
            <div style="background-color:#ffe0f0; padding:30px; border:4px solid #b00072; border-radius:20px; text-align:center; color:#b00072; box-shadow: 0 4px 15px rgba(255, 105, 180, 0.7);">
                ...
                <p style="font-size:1.4em; font-weight:normal; line-height: 1.5; font-family: 'Dancing Script', cursive;">{st.session_state['wheel_result']}</p>
            </div>
        """, unsafe_allow_html=True)
        st.markdown("---")
        col1, col2 = st.columns(2)
...
def page_celebration_room():
    ...
    if not st.session_state['candles_blown']:
        ...
        <div style="display: flex; justify-content: center; margin-bottom: 20px;">
            <script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script>
            <lottie-player src="https://lottie.host/79a4e402-9ff0-4a81-995b-0045d47d4834/5s1F3b1v8y.json" background="transparent" speed="1" style="width: 250px; height: 250px;" autoplay></lottie-player>
        </div>
        """
        ...
        st.markdown("""
            <div style="background-color:#fff0f5; padding:30px; border:4px dashed #ff99aa; border-radius:15px; text-align:center; color:#b00072;">
                <p style="font-size:2.0em; color:#b00072; font-weight:bold; margin-bottom: 5px; font-family:'Chewy', cursive;">Happy Birthday, My Aaadhi!</p>
                <p style="font-size:1.4em; color:#b00072; font-family: 'Dancing Script', cursive;">I wish you the greatest day filled with joy, laughter, and endless love. <br> You deserve all the happiness in the world. <br> I love you, now and always! 💖</p>
            </div>
        """, unsafe_allow_html=True)
    st.markdown("---")
...
def page_admin_view():
    st.title("🤫 Admin View: Captured Responses")
    st.warning("🚨 WARNING: These responses are only visible while the app is running in this current session. If the app or server restarts, they will be lost!")
    st.markdown("---")
    st.subheader("Future Image Choice 🔮")
    if st.session_state.get('favorite_future_image_saved'):
        st.success("**Saved Future Image:** %s" % st.session_state.get('favorite_future_image'))
    else:
        st.info("Future Image not yet saved.")
    st.subheader("Message Back 💌")
    if st.session_state.get('message_status') == 'sent':
        st.markdown("**From:** `%s`" % st.session_state.get('your_name', 'N/A'))
        st.info("**Message:** \n\n%s" % st.session_state.get('message', 'No message submitted.'))
    else:
        st.info("Message Back not yet submitted.")
    st.subheader("This or That Responses ❓")
    q_map = {q['key']: q['question'] for q in THIS_OR_THAT_QUESTIONS}
    responses = st.session_state.get('this_or_that_responses', {})
    if responses:
        for key, value in responses.items():
            q_text = q_map.get(key, key)
            st.markdown("**%s:** `%s`" % (q_text, value))
    else:
        st.info("This or That not yet completed.")
    st.markdown("---")
    st.button("⬅️ Back to Home", use_container_width=True, on_click=lambda: set_page('home', 'admin_view'))

def main():
    defaults = {
        PAGE_KEY: 'home', 'prev_page': 'home', 'message_status': '', 'hearts_caught': 0,
        'candles_blown': False, 'correct_spot_index': random.randint(0, 8),
        'current_love_message': random.choice(LOVE_MESSAGES), 'wheel_state': 'initial',
        'wheel_result': None, 'favorite_future_image': None, 'favorite_future_image_saved': False,
        'this_or_that_responses': {}, 'this_or_that_saved': False, 'bg_volume': 0.4
    }
