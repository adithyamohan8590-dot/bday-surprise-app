...
def page_home():
    ...
    cols3 = st.columns(1)
    cols3[0].button("🤫 Admin View (For Me Only)", use_container_width=True, on_click=lambda: set_page('admin_view', 'home'))
...
def page_voice():
    ...
    if voice_path:
        # FIX: Added the key="voice_audio" for the JavaScript to correctly target this element.
        st.audio(voice_path, format="audio/mp3", key="voice_audio") 
        ...
        <script>
            ...
            // The querySelector now correctly targets the element with the 'voice_audio' key
...
def page_future():
    ...
    # Message shown beside the save button using columns
...
def page_catch_hearts():
    ...
    # Ensure correct_spot_index is initialized (was already handled in main, but good to check)
    ...
    # START: Custom wrapper for small buttons
    ...
    # END: Custom wrapper for small buttons
...
def page_admin_view():
    ...
    # Create a mapping for question keys to full text
...
def main():
    ...
    page_functions = {
        ...
        'admin_view': page_admin_view # ADDED ADMIN VIEW
    }
