...
# NOTE: Ensure all referenced files are in the same directory as this Python file.
...
def set_custom_theme(collage_file, music_file):
    ...
    # Get the current volume from session state for JavaScript
    ...
    html_content = f"""
        ...
        /* NEW: Button Style for Small Grid Buttons (Catch Hearts) */
        .heart-grid-wrapper .stButton > button {{
            background-color: #ff99aa; 
            border: 1px solid #ffb6c1; 
            border-radius: 8px;
            padding: 5px 5px;
            font-size: 0.85em; 
            height: 60px; /* Define size */
            line-height: 1.2;
        }}
        ...
        if (music) {{
            music.volume = {current_volume}; // Use the volume set by Streamlit slider
...
def page_home():
    ...
    # Volume slider removed from here and moved to page_voice
    ...
    cols3 = st.columns(1)
    # The Admin View is a secret link, only you would know to click
...
def page_voice():
    ...
    # NEW: Volume Slider and Message added here
    ...
    new_volume = st.slider(
        ...
        step=0.05,
        format="%.2f",
        key='voice_page_volume_slider', # Unique key
        help="Adjust this volume. The music will pause when the voice note plays, and resume at this level when it finishes."
    )
    
    if new_volume != current_volume:
        st.session_state['bg_volume'] = new_volume
        # Rerunning to ensure the JS variable in set_custom_theme is updated
        st.rerun() 
        
    st.markdown("---")
    
    if voice_path:
        # Note: We use a custom key here so that Streamlit re-creates the audio component 
        # when st.rerun() happens, which ensures the new volume takes effect immediately 
        # on the background music without needing a page refresh.
        ...
        <script>
            ...
            // Selects the audio element inside the st.audio component
            const voiceAudio = document.querySelector('[data-testid="stAudio"] audio');
            
            if (bgMusic && voiceAudio) {{
                
                // 1. PAUSE background music on voice note play
                voiceAudio.onplay = function() {{
                    bgMusic.pause(); // PAUSE the background music completely
                }};
                
                // 2. Function to safely RESUME background music
                const resumeMusic = function() {{
                    // Only resume if the voice note is not currently playing
                    if (voiceAudio.paused || voiceAudio.ended) {{ 
                        // Restore volume using the Streamlit-set value (which is the new slider value on rerun)
                        bgMusic.volume = {current_volume}; 
                        bgMusic.play().catch(e => console.log("BG music resume failed:", e));
                    }}
                }};

                // 3. Resume background music when voice note ends
                voiceAudio.onended = resumeMusic;
                
                // 4. Resume background music if user manually pauses voice note
                voiceAudio.onpause = resumeMusic;
            }}
        </script>
        """
        ...
        st.error(f"❌ Error: The voice note file **'{VOICE_FILE}'** was not found.")
...
# --- ADMIN PAGE: TO CHECK AND 'SAVE' RESPONSES ---
...
def page_admin_view():
    ...
    if st.session_state.get('favorite_future_image_saved'):
        st.success(f"**Saved Future Image:** {st.session_state['favorite_future_image']}")
    else:
        st.info("Future Image not yet saved.")

    st.subheader("Message Back 💌")
    if st.session_state.get('message_status') == 'sent':
        st.markdown(f"**From:** `{st.session_state.get('your_name', 'N/A')}`")
        st.info(f"**Message:** \n\n{st.session_state.get('message', 'No message submitted.')}")
    else:
...
def main():
    ...
    defaults = {
        ...
        'bg_volume': 0.4 # Default background music volume
    }
